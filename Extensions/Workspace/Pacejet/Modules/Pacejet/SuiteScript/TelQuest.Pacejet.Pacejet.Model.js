define('TelQuest.Pacejet.Pacejet.Model', [
    'Configuration',
    'TelQuest.Pacejet.Pacejet.Configuration',
    'TelQuest.Pacejet.Pacejet.DataSource.LiveOrder',
    'TelQuest.Pacejet.Pacejet.RequestBuilder',
    'TelQuest.Pacejet.Pacejet.Utils',
    'TelQuest.Pacejet.Pacejet.Carrier.Model',
    'SC.Models.Init',
    'underscore'
    ...
    return {
        ...
        markFreeShipping: function markFreeShipping(requestData, responseData) {
            ...
            if (requestData && requestData.transaction && requestData.transaction.freeshipping === true) {
                sm = _.findWhere(responseData.data.shipmethods, { xref: Configuration.get().pacejet.freeshipping });
                if (sm) {
                    sm.rate = 0;
                }
            }
        ...
        parseResponse: function parseResponse(data) {
            ...
            filteredMethods = _.filter(originalShipmethodsData, function onFilter(method) {
				nlapiLogExecution('debug','method',JSON.stringify(method));
