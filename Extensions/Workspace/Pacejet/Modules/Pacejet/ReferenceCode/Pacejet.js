...
,	_getRates: function (shipaddress, order)
	{
		...
		try {
			...
			nlapiLogExecution('debug', '_getRates: get cache', JSON.stringify(cache,null,2));
...
,	_updateShippingRates: function (results, order, data) 
	{
		...
		try {
			...
			if (shipaddress) {
				...
				nlapiLogExecution('debug', 'ratingResultsList',JSON.stringify(ratingResultsList));
				...
				//  else set the summary rate to the lowest rate
					...
					var lowestRate = _.chain(ratingResultsList).sortBy('listFreight').first().value();
					...
					results.summary.total = results.summary.total - results.summary.shippingcost + lowestRate.listFreight; 
					results.summary.total_formatted = formatCurrency(results.summary.total);
					//results.summary.shippingcost = lowestRate.consigneeFreight;
					results.summary.shippingcost = lowestRate.listFreight;
					results.summary.shippingcost_formatted = formatCurrency(results.summary.shippingcost);
				}
			}
...
,	_filterShippingMethods: function(results, ratingResultsList) 
	{
		...
		nlapiLogExecution('debug', '_filterShippingMethods: ratingResultsList.length', JSON.stringify(ratingResultsList));
		...
		if (results && results.shipmethods && results.shipmethods.length) {
			...
			_.each(ratingResultsList, function (e) {
				...
				ratingHash[e.internalid] = e.listFreight;
			});
