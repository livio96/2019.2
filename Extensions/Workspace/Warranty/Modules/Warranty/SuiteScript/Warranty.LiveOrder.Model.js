define('TelQuest.Warranty.Warranty.LiveOrder.Model', [
    ...
    if (Configuration.get().warranty.itemOption) {
        LiveOrderModel.addToCartFunctions = LiveOrderModel.addToCartFunctions || {};
        LiveOrderModel.addToCartFunctions[Configuration.get().warranty.itemOption] = 'addToCartWarranty';
    }

    Application.on('before:LiveOrder.updateLine',
        function beforeLiveOrderUpdateLine(Model, lineid, line) {
            var warrantySelected = Configuration.get().warranty.itemOption;
            var warrantyAssociation = Configuration.get().warranty.itemOptionAssociation;
    ...
    _.extend(LiveOrderModel, {
        addToCartWarranty: function addToCartWarranty(currentLine, newLines) {
            ...
            var generatedIdNL;
            var generatedIdOld;
            var addOnItemId = Configuration.get().warranty.itemId;
            var warrantySelected = Configuration.get().warranty.itemOption;
            var warrantyAssociation = Configuration.get().warranty.itemOptionAssociation;
            var options = [];
