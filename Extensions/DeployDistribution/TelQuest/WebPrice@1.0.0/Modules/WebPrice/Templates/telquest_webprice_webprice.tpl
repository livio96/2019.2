...
{{#if MSRPPrice}}
    <small class="product-views-price-msrp" style="display:none;">
        {{translate 'MSRP'}}: {{MSRPPrice}}
    </small>
{{/if}}
{{#if webitemavailability}}
        <p class="out-of-stock-available">{{webitemavailability}}</p>
    {{/if}}
	{{#if webinstock}}
        <p class="stock-available"><i class="fa fa-check"></i> {{webinstock}}</p>
    {{/if}}
	{{#if isPriceEnabled}}
		{{#if isPriceRange}}
