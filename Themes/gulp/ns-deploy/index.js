	...
	return through.through(
		...
		, function ()
		{
			var waterfall = [
				...
				, fs.processFiles
				, net.postFiles
				, package_manager.distro.isSCLite ? net.writeConfig : function (distro, cb2)
				{
					cb2(null, distro);
				}
			];
	...
	async.doUntil(
		...
		function ()
		{
			...
			if (deploy.folder_inquirer && deploy.folder_inquirer.target_folder)
			{
				...
				var publisher_apps = deploy.folder_inquirer.target_publisher_folder.publisher.application;
				var folder_name = publisher_apps.find((folder)=> deploy.info.target_folder === folder.id).name;
